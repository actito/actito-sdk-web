import { updateCloudDevice } from '@actito/web-cloud-api';
import {
  getCurrentDevice,
  getOptions,
...
export function startLocationUpdates() {
  ...
  if (!options) {
    logger.warning('Cannot start location updates before Actito is configured.');
    return;
  }
...
function createLocation(position: GeolocationPosition): ActitoLocation {
  return {

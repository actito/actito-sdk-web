import { fetchCloudInAppMessage } from '@actito/web-cloud-api';
import {
...
function processMessage(message: ActitoInAppMessage) {
...
function presentMessage(message: ActitoInAppMessage) {
  if (isShowingMessage()) {
...
async function fetchInAppMessage(context: ApplicationContext): Promise<ActitoInAppMessage> {
  const device = getCurrentDevice();

...
const ACTITO_CORE_URL = `https://cdn.notifica.re/libs/web/v5/${pkg.version}/actito-core.js`;
const ACTITO_LATEST_CORE_URL = `https://cdn.notifica.re/libs/web/v5/latest/actito-core.js`;
const ACTITO_INTERNAL_CORE_URL = `https://cdn.notifica.re/libs/web/internal/${pkg.version}/actito-core.js`;
...
function bundleSources(variant) {
  ...
  const files = pkg.components.map((component) => {
    ...
    return `.build/min/actito-${componentName}.js`;
  });

  return gulp.src(files)
    .pipe(sourcemaps.init({ loadMaps: true }))
    .pipe(replace(/(['"])@actito\/web-core(['"])/g, `$1${coreUrl}$2`))
...
function bundleCss(variant) {
  ...
  return gulp.src(files)
    ...
    .pipe(rename(function(path) {
      path.basename = `actito-${path.basename}`;
    }))
    .pipe(gulp.dest(destination));
}
...
function getVariantInfo(variant) {
  switch (variant) {
    case "version":
      return {
        coreUrl: ACTITO_CORE_URL,
        destination: ".build/cdn-version"
      };
    case "latest":
      return {
        coreUrl: ACTITO_LATEST_CORE_URL,
        destination: ".build/cdn-latest"
      };
    case "internal":
      return {
        coreUrl: ACTITO_INTERNAL_CORE_URL,
...
export function uploadVersionRelease() {
  ...
  return gulp.src(`${destination}/**`)
    .pipe(s3({
      ...
      ACL: "public-read",
      keyTransform: (filename) => `libs/web/v5/${pkg.version}/${filename}`
    }));
}
...
export function uploadLatestRelease() {
  ...
  return gulp.src(`${destination}/**`)
    .pipe(s3({
      Bucket: "cdn.notifica.re",
      ACL: "public-read",
      keyTransform: (filename) => `libs/web/v5/latest/${filename}`
    }));
}

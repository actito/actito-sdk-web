...
export async function createVideoContent(notification: ActitoNotification): Promise<HTMLElement> {
...
function createYouTubeContent(videoId: string): HTMLElement {
  ...
  iframe.classList.add('actito__notification-video-iframe');
...
function createHtml5VideoContent(videoUrl: string): HTMLElement {
  ...
  video.classList.add('actito__notification-video');

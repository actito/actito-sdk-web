...
export async function createMapContent(notification: ActitoNotification): Promise<HTMLElement> {
  ...
  mapContainer.classList.add('actito__notification-map');
  ...
  const markers = content.map(({ data }) => {
    ...
    if (data.title || data.description) {
      ...
      infoWindowContent.classList.add('actito__notification-map-info-window');

      if (data.title) {
        const title = document.createElement('h2');
        title.classList.add('actito__notification-map-info-window-title');
      ...
      if (data.description) {
        ...
        description.classList.add('actito__notification-map-info-window-description');

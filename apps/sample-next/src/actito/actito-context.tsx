...
import { launch, onDeviceRegistered, onReady, onUnlaunched, unlaunch } from "actito-web/core";
import { onLocationUpdated, onLocationUpdateError } from "actito-web/geo";
import {
...
import { onBadgeUpdated, onInboxUpdated } from "actito-web/inbox";
import {
...
export function useActito() {
  const context = useContext(ActitoContext);

  if (!context) {
    throw new Error("Unable to find the ActitoProvider in the component tree.");
  }

  return context;
}

export function ActitoProvider({ children }: PropsWithChildren) {
  ...
  const contextState = useMemo<ActitoContextState>(
    () => ({
      state,
      launch: launchFn,
      unlaunch: unlaunchFn,
  ...
  return <ActitoContext.Provider value={contextState}>{children}</ActitoContext.Provider>;
}

type ActitoContextState = {
  state: LaunchState;
  launch: LaunchFn;
  unlaunch: UnlaunchFn;
